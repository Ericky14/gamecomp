//! X11 atom definitions and registry.
//!
//! Defines all custom atoms used for runtime compositor control. Atoms are interned once at XWM startup
//! and stored for the lifetime of the connection.
//!
//! # Design
//!
//! Atoms are organized by category:
//! - **Window properties**: set on individual X11 windows to classify them
//!   (game, overlay, notification, etc.)
//! - **Root properties**: set on the root window by external controllers
//!   (Steam, MangoHud, scripts) to control compositor behavior at runtime
//! - **Feedback properties**: written by the compositor back to the root window
//!   so controllers can read current state
//!
//! Atom naming uses gamescope-compatible prefixes for ecosystem interop
//! (gamescope-dbus, playserve, Steam):
//! - `GAMESCOPE_*` — compositor feedback / display state atoms
//! - `GAMESCOPECTRL_*` — control atoms (written by external clients)
//! - `STEAM_*` — Steam classification atoms

use anyhow::Context;
use tracing::info;
use x11rb::connection::Connection;
use x11rb::protocol::xproto::ConnectionExt;

/// All interned atoms, grouped by category.
///
/// Created once per XWM connection via [`Atoms::intern`].
#[derive(Debug)]
pub struct Atoms {
    // --- Standard atoms ---
    /// `WL_SURFACE_ID` — links X11 window to its Wayland wl_surface.
    pub wl_surface_id: u32,
    /// `_NET_WM_NAME` — window title (UTF-8).
    pub net_wm_name: u32,
    /// `_NET_WM_PID` — window process ID.
    pub net_wm_pid: u32,
    /// `_NET_WM_STATE` — window state list.
    pub net_wm_state: u32,
    /// `_NET_WM_STATE_FULLSCREEN` — fullscreen state flag.
    pub net_wm_state_fullscreen: u32,
    /// `_NET_WM_STATE_SKIP_TASKBAR` — window should be skipped in taskbar.
    pub net_wm_state_skip_taskbar: u32,
    /// `_NET_WM_STATE_SKIP_PAGER` — window should be skipped in pager.
    pub net_wm_state_skip_pager: u32,
    /// `WM_TRANSIENT_FOR` — parent window for transient windows.
    pub wm_transient_for: u32,
    /// `WM_NORMAL_HINTS` — size hints (min/max size, aspect ratio).
    pub wm_normal_hints: u32,
    /// `_NET_WM_WINDOW_TYPE` — window type.
    pub net_wm_window_type: u32,
    /// `_NET_WM_WINDOW_TYPE_NORMAL` — normal window type.
    pub net_wm_window_type_normal: u32,
    /// `_NET_WM_WINDOW_TYPE_DIALOG` — dialog window type.
    pub net_wm_window_type_dialog: u32,
    /// `_NET_WM_WINDOW_TYPE_POPUP_MENU` — popup menu window type.
    pub net_wm_window_type_popup: u32,
    /// `_NET_WM_OPACITY` — window opacity (0–0xFFFFFFFF).
    pub net_wm_opacity: u32,
    /// `WM_DELETE_WINDOW` — close window protocol atom.
    pub wm_delete_window: u32,
    /// `WM_PROTOCOLS` — supported protocols list.
    pub wm_protocols: u32,
    /// `UTF8_STRING` — UTF-8 string type atom.
    pub utf8_string: u32,

    // --- Window classification atoms (set on individual windows) ---
    /// `STEAM_GAME` — Steam AppID for game windows.
    pub steam_game: u32,
    /// `STEAM_OVERLAY` — marks a window as a Steam overlay.
    pub steam_overlay: u32,
    /// `STEAM_BIGPICTURE` — marks a window as the Steam client.
    pub steam_bigpicture: u32,
    /// `STEAM_INPUT_FOCUS` — input focus mode (0=normal, 1=overlay grabs).
    pub steam_input_focus: u32,
    /// `GAMESCOPE_EXTERNAL_OVERLAY` — external overlay (MangoHud, etc.).
    pub external_overlay: u32,

    // --- Focus control atoms (set on root by gamescope-dbus / Steam) ---
    /// `GAMESCOPECTRL_BASELAYER_APPID` — array of AppIDs to focus.
    pub focus_appid: u32,
    /// `GAMESCOPECTRL_BASELAYER_WINDOW` — specific window ID to focus.
    pub focus_window: u32,
    /// `GAMESCOPE_FOCUS_DISPLAY` — which display receives focus.
    pub focus_display: u32,
    /// `GAMESCOPE_KEYBOARD_FOCUS_DISPLAY` — set on the primary XWayland root
    pub keyboard_focus_display: u32,

    // --- Resolution / display atoms (set on root) ---
    /// `GAMESCOPE_XWAYLAND_MODE_CONTROL` — `[server_idx, width, height, flags]`.
    pub xwayland_mode_control: u32,
    /// `GAMESCOPE_FORCE_WINDOWS_FULLSCREEN` — force all windows fullscreen.
    pub force_windows_fullscreen: u32,

    // --- FPS / performance atoms (set on root) ---
    /// `GAMESCOPE_FPS_LIMIT` — runtime FPS cap.
    pub fps_limit: u32,
    /// `GAMESCOPE_VRR_ENABLED` — enable variable refresh rate.
    pub vrr_enabled: u32,
    /// `GAMESCOPE_LOW_LATENCY` — enable low-latency mode.
    pub low_latency: u32,
    /// `GAMESCOPE_ALLOW_TEARING` — allow tearing for VRR.
    pub allow_tearing: u32,

    // --- Scaling / filter atoms (set on root) ---
    /// `GAMESCOPE_SCALING_FILTER` — scaling filter selection.
    pub scaling_filter: u32,
    /// `GAMESCOPE_FSR_SHARPNESS` — FSR sharpness (0–20).
    pub fsr_sharpness: u32,
    /// `GAMESCOPE_SHARPNESS` — general sharpness (0–20).
    pub sharpness: u32,

    // --- HDR atoms (set on root) ---
    /// `GAMESCOPE_HDR_ENABLED` — enable HDR output.
    pub hdr_enabled: u32,
    /// `GAMESCOPE_SDR_ON_HDR_BRIGHTNESS` — SDR content brightness in HDR mode.
    pub sdr_on_hdr_brightness: u32,

    // --- Compositor debug atoms (set on root) ---
    /// `GAMESCOPE_COMPOSITE_FORCE` — force full composition (no direct scanout).
    pub composite_force: u32,
    /// `GAMESCOPE_COMPOSITE_DEBUG` — debug visualization flags.
    pub composite_debug: u32,

    // --- Multi-display atoms (set on root) ---
    /// `GAMESCOPE_XWAYLAND_SERVER_ID` — published on each server's root window
    /// so gamescope-dbus can discover and manage XWayland instances.
    pub xwayland_server_id: u32,
    /// `GAMESCOPE_CREATE_XWAYLAND_SERVER` — request new XWayland server.
    pub create_xwayland_server: u32,
    /// `GAMESCOPE_CREATE_XWAYLAND_SERVER_FEEDBACK` — feedback after dynamic server
    /// creation. Value is `"<identifier> <server_id> <display_name>"`.
    pub create_xwayland_server_feedback: u32,
    /// `GAMESCOPE_DESTROY_XWAYLAND_SERVER` — destroy XWayland server by ID.
    pub destroy_xwayland_server: u32,

    // --- Feedback atoms (written by compositor to root for controllers to read) ---
    /// `GAMESCOPE_FOCUSED_APP` — currently focused AppID.
    pub focused_app: u32,
    /// `GAMESCOPE_FOCUSED_WINDOW` — currently focused window ID.
    pub focused_window: u32,
    /// `GAMESCOPE_FOCUSABLE_APPS` — list of all focusable AppIDs.
    pub focusable_apps: u32,
    /// `GAMESCOPE_FOCUSABLE_WINDOWS` — triplets [windowID, appID, pid].
    pub focusable_windows: u32,
    /// `GAMESCOPE_VRR_CAPABLE` — whether display supports VRR.
    pub vrr_capable: u32,
    /// `GAMESCOPE_VRR_IN_USE` — whether VRR is currently active.
    pub vrr_in_use: u32,
    /// `GAMESCOPE_HDR_SUPPORTED` — whether display supports HDR.
    pub hdr_supported: u32,
    /// `GAMESCOPE_FSR_ACTIVE` — whether FSR is currently active.
    pub fsr_active: u32,
    /// `GAMESCOPE_DISPLAY_REFRESH_RATE` — current display refresh rate.
    pub display_refresh_rate: u32,
    /// `GAMESCOPE_CURSOR_VISIBLE` — cursor visibility state.
    pub cursor_visible: u32,
    /// `GAMESCOPE_PID` — compositor process ID.
    pub pid: u32,

    // --- Screenshot ---
    /// `GAMESCOPECTRL_REQUEST_SCREENSHOT` — trigger screenshot.
    pub request_screenshot: u32,
}

impl Atoms {
    /// Intern all atoms on the given X11 connection.
    ///
    /// Sends all `InternAtom` requests in a single batch for efficiency,
    /// then collects all replies. This is much faster than interning
    /// one-by-one (one round-trip vs N round-trips).
    pub fn intern<C: Connection>(conn: &C) -> anyhow::Result<Self> {
        let names: &[&str] = &[
            // Standard
            "WL_SURFACE_ID",
            "_NET_WM_NAME",
            "_NET_WM_PID",
            "_NET_WM_STATE",
            "_NET_WM_STATE_FULLSCREEN",
            "_NET_WM_STATE_SKIP_TASKBAR",
            "_NET_WM_STATE_SKIP_PAGER",
            "WM_TRANSIENT_FOR",
            "WM_NORMAL_HINTS",
            "_NET_WM_WINDOW_TYPE",
            "_NET_WM_WINDOW_TYPE_NORMAL",
            "_NET_WM_WINDOW_TYPE_DIALOG",
            "_NET_WM_WINDOW_TYPE_POPUP_MENU",
            "_NET_WM_OPACITY",
            "WM_DELETE_WINDOW",
            "WM_PROTOCOLS",
            "UTF8_STRING",
            // Window classification
            "STEAM_GAME",
            "STEAM_OVERLAY",
            "STEAM_BIGPICTURE",
            "STEAM_INPUT_FOCUS",
            "GAMESCOPE_EXTERNAL_OVERLAY",
            // Focus control
            "GAMESCOPECTRL_BASELAYER_APPID",
            "GAMESCOPECTRL_BASELAYER_WINDOW",
            "GAMESCOPE_FOCUS_DISPLAY",
            "GAMESCOPE_KEYBOARD_FOCUS_DISPLAY",
            // Resolution / display
            "GAMESCOPE_XWAYLAND_MODE_CONTROL",
            "GAMESCOPE_FORCE_WINDOWS_FULLSCREEN",
            // FPS / performance
            "GAMESCOPE_FPS_LIMIT",
            "GAMESCOPE_VRR_ENABLED",
            "GAMESCOPE_LOW_LATENCY",
            "GAMESCOPE_ALLOW_TEARING",
            // Scaling / filter
            "GAMESCOPE_SCALING_FILTER",
            "GAMESCOPE_FSR_SHARPNESS",
            "GAMESCOPE_SHARPNESS",
            // HDR
            "GAMESCOPE_HDR_ENABLED",
            "GAMESCOPE_SDR_ON_HDR_BRIGHTNESS",
            // Debug
            "GAMESCOPE_COMPOSITE_FORCE",
            "GAMESCOPE_COMPOSITE_DEBUG",
            // Multi-display
            "GAMESCOPE_XWAYLAND_SERVER_ID",
            "GAMESCOPE_CREATE_XWAYLAND_SERVER",
            "GAMESCOPE_CREATE_XWAYLAND_SERVER_FEEDBACK",
            "GAMESCOPE_DESTROY_XWAYLAND_SERVER",
            // Feedback
            "GAMESCOPE_FOCUSED_APP",
            "GAMESCOPE_FOCUSED_WINDOW",
            "GAMESCOPE_FOCUSABLE_APPS",
            "GAMESCOPE_FOCUSABLE_WINDOWS",
            "GAMESCOPE_VRR_CAPABLE",
            "GAMESCOPE_VRR_IN_USE",
            "GAMESCOPE_HDR_SUPPORTED",
            "GAMESCOPE_FSR_ACTIVE",
            "GAMESCOPE_DISPLAY_REFRESH_RATE",
            "GAMESCOPE_CURSOR_VISIBLE",
            "GAMESCOPE_PID",
            // Screenshot
            "GAMESCOPECTRL_REQUEST_SCREENSHOT",
        ];

        // Send all requests in one batch (pipelined, single flush).
        let cookies: Vec<_> = names
            .iter()
            .map(|name| conn.intern_atom(false, name.as_bytes()))
            .collect::<Result<Vec<_>, _>>()
            .context("failed to send InternAtom requests")?;

        // Collect all replies.
        let atoms: Vec<u32> = cookies
            .into_iter()
            .map(|cookie| cookie.reply().map(|r| r.atom))
            .collect::<Result<Vec<_>, _>>()
            .context("failed to collect InternAtom replies")?;

        info!(count = atoms.len(), "interned X11 atoms");

        Ok(Self {
            // Standard
            wl_surface_id: atoms[0],
            net_wm_name: atoms[1],
            net_wm_pid: atoms[2],
            net_wm_state: atoms[3],
            net_wm_state_fullscreen: atoms[4],
            net_wm_state_skip_taskbar: atoms[5],
            net_wm_state_skip_pager: atoms[6],
            wm_transient_for: atoms[7],
            wm_normal_hints: atoms[8],
            net_wm_window_type: atoms[9],
            net_wm_window_type_normal: atoms[10],
            net_wm_window_type_dialog: atoms[11],
            net_wm_window_type_popup: atoms[12],
            net_wm_opacity: atoms[13],
            wm_delete_window: atoms[14],
            wm_protocols: atoms[15],
            utf8_string: atoms[16],
            // Window classification
            steam_game: atoms[17],
            steam_overlay: atoms[18],
            steam_bigpicture: atoms[19],
            steam_input_focus: atoms[20],
            external_overlay: atoms[21],
            // Focus control
            focus_appid: atoms[22],
            focus_window: atoms[23],
            focus_display: atoms[24],
            keyboard_focus_display: atoms[25],
            // Resolution / display
            xwayland_mode_control: atoms[26],
            force_windows_fullscreen: atoms[27],
            // FPS / performance
            fps_limit: atoms[28],
            vrr_enabled: atoms[29],
            low_latency: atoms[30],
            allow_tearing: atoms[31],
            // Scaling / filter
            scaling_filter: atoms[32],
            fsr_sharpness: atoms[33],
            sharpness: atoms[34],
            // HDR
            hdr_enabled: atoms[35],
            sdr_on_hdr_brightness: atoms[36],
            // Debug
            composite_force: atoms[37],
            composite_debug: atoms[38],
            // Multi-display
            xwayland_server_id: atoms[39],
            create_xwayland_server: atoms[40],
            create_xwayland_server_feedback: atoms[41],
            destroy_xwayland_server: atoms[42],
            // Feedback
            focused_app: atoms[43],
            focused_window: atoms[44],
            focusable_apps: atoms[45],
            focusable_windows: atoms[46],
            vrr_capable: atoms[47],
            vrr_in_use: atoms[48],
            hdr_supported: atoms[49],
            fsr_active: atoms[50],
            display_refresh_rate: atoms[51],
            cursor_visible: atoms[52],
            pid: atoms[53],
            // Screenshot
            request_screenshot: atoms[54],
        })
    }
}
